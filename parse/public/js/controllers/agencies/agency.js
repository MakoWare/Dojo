'use strict';
//Agency Ctrl
var AgencyCtrl = function($rootScope, $scope, $location, ParseService, GlobalService, $modal){
    ...
    $scope.init = function(){
        ...
        if(last == "create"){
            $scope.action = "Create";
            $scope.title = "New Agency";
            $scope.createAgency();
        } else {
            $scope.action = "Update";
            $scope.title = "Update";
            //$scope.getAgency(last);
        }
    };

    //Create Agency
    $scope.createAgency = function(){
        ParseService.createObject("Agency", function(results){
            setTimeout(function(){
                console.log(results);
                $scope.agency = results;
                $scope.setUpUser();
            }, 500);
        });
    };
    ...
    $scope.getAgency = function(id){
        ParseService.getObjectById("Agency", id, function(results){
            if(results.id){
                console.log(results);
                $scope.agency = results;
